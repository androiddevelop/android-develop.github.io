    ...
    function () {
        ...
        _createClass(AutocJs, [{
            ...
            value: function generateChapters(headings) {
                ...
                headings.forEach(function (heading, i) {
                    ...
                    // h3 （当前标题：这种情况我们还是任务 h3 是 h2 的下一级章节）
                        ...
                        // h2
                        // h3
                        // h2 这一级标题的处理，应该和上一个h2是一级别


                        if (current < previous && current === lastParent) {
                            level = level - 1;
            ...
            value: function renderOutsideOutline() {
                ...
                var dom = utils.DOM;
                var elements = this.getElements();
                var title = this.get('title'); // 创建导航头部
                ...
                elements.footer = dom.createElement('div', {
                    ...
                    id: 'cb-menu'
                }, [elements.switcher, elements.top]); // 创建导航菜单主体框架

                elements.modal = dom.createElement('div', {
                    className: 'outline-outside-modal'
                }, [elements.header, elements.body, elements.footer]); // 创建导航菜单遮罩层
